...
{ ## DE PLOT FUNCTIONS
  ...
  volcanoPlot <- function(stats, comparison=NULL, min.pval=0.055, min.lfc=1,
    ...
    ## add points
    with(stats, points(logFC, -log10(p), pch=21, bg=scales::alpha(colour="grey60",alpha=0.5), ## black
                       col=scales::alpha(colour="grey50",alpha=0.5), lwd=0.5, cex=1.1))
    with(up, points(logFC, -log10(p), pch=21, bg=scales::alpha(colour="#FF0000",alpha=1),     ## red
  ...
  mdPlot <- function(stats, comparison=NULL, min.pval=0.055, min.lfc=1,
    ...
    ## add points
    with(stats, points(AveExpr, logFC, pch=21, bg=scales::alpha(colour="grey60",alpha=0.5), ## black
                       col=scales::alpha(colour="grey50",alpha=0.5), lwd=0.5, cex=1.1))
    with(up, points(AveExpr,logFC, pch=21, bg=scales::alpha(colour="#FF0000",alpha=1),     ## red
                    col=scales::alpha(colour="#990000",alpha=1), lwd=0.5, cex=1.1))
  ...
  glimmaMDPlot <- function(stats, comparison, data, annot, groups, min.pval=0.05, min.lfc=1,
    ...
    Glimma::glXYPlot(x         = stats$AveExpr,
             y         = stats$logFC,
             counts    = data[rownames(stats), ],
             groups    = groups,
             samples   = colnames(data),
