#main mismatch we know about from the check log is the class of the expected objects:
# your functions now return class(x) == c("DAList", "list"), 
# but the fixtures were saved when it was just "DAList". 
# That’s exactly the pattern we fixed in test-add_design.R.
#So first pass: update the tests that compare full objects to coerce the fixtures’ class before expect_equal().
...
test_that("filter_proteins_by_annotation removes proteins as expected", {
  ...
  class(output_ID)   <- c("DAList", "list")
  class(output_name) <- c("DAList", "list")
  class(output_MW)   <- c("DAList", "list")
  
  expect_equal(
    filter_proteins_by_annotation(input, uniprot_id != "protein10"),
    output_ID
  )
  expect_equal(
    filter_proteins_by_annotation(input, !stringr::str_detect(Protein.Name, "keratin")),
    output_name
  )
  expect_equal(
    filter_proteins_by_annotation(input, molecular_weight < 10),
    output_MW
  )
})
...
test_that("filter_proteins_by_group checks input contains metadata", {
  ...
  expect_error(
    filter_proteins_by_group(input, min_reps = 1, min_groups = 1),
...
test_that("filter_proteins_by_groups gives error when group column is not present", {
  ...
  expect_error(
    filter_proteins_by_group(input, grouping_column = "xxx", min_reps = 10, min_groups = 3),
    "not found"
  )
})
...
test_that("filter_proteins_by_groups gives error when group and individual thresholds are too high", {
  ...
  expect_error(
    filter_proteins_by_group(input, min_reps = 10, min_groups = 3),
    "Groups below threshold"
  )
  expect_error(
    filter_proteins_by_group(input, min_reps = 4, min_groups = 4),
    "Not enough groups"
  )
})
...
test_that("filter_proteins_by_groups removes proteins as expected", {
  ...
  # Update fixtures to current DAList class structure
  class(output_13) <- c("DAList", "list")
  class(output_21) <- c("DAList", "list")
  class(output_23) <- c("DAList", "list")
  class(output_33) <- c("DAList", "list")
  class(output_43) <- c("DAList", "list")
  
  suppressMessages(
    expect_equal(
      filter_proteins_by_group(input, min_reps = 1, min_groups = 3),
      output_13
    )
  )
  suppressMessages(
    expect_equal(
      filter_proteins_by_group(input, min_reps = 2, min_groups = 1),
      output_21
    )
  )
  suppressMessages(
    expect_equal(
      filter_proteins_by_group(input, min_reps = 2, min_groups = 3),
      output_23
    )
  )
  suppressMessages(
    expect_equal(
      filter_proteins_by_group(input, min_reps = 3, min_groups = 3),
      output_33
    )
  )
  suppressMessages(
    expect_equal(
      filter_proteins_by_group(input, min_reps = 4, min_groups = 3),
      output_43
    )
  )
})
...
test_that("filter_proteins_by_proportion checks input contains metadata", {
  ...
  input["metadata"] <- list(NULL)
  expect_error(
    filter_proteins_by_proportion(input, min_prop = 0.7),
    "must include metadata"
  )
})
...
test_that("filter_proteins_by_proportion gives error when group column is not present", {
  ...
  expect_error(
    filter_proteins_by_proportion(input, grouping_column = "xxx", min_prop = 0.5),
    "not found"
  )
})
...
test_that("filter_proteins_by_proportion removes proteins as expected", {
  ...
  # Update fixtures to current DAList class structure
  class(output_0)  <- c("DAList", "list")
  class(output_25) <- c("DAList", "list")
  class(output_50) <- c("DAList", "list")
  class(output_75) <- c("DAList", "list")
  class(output_1)  <- c("DAList", "list")
  
  suppressMessages(
    expect_equal(
      filter_proteins_by_proportion(input, min_prop = 0),
      output_0
    )
  )
  suppressMessages(
    expect_equal(
      filter_proteins_by_proportion(input, min_prop = 0.25),
      output_25
    )
  )
  suppressMessages(
    expect_equal(
      filter_proteins_by_proportion(input, min_prop = 0.5),
      output_50
    )
  )
  suppressMessages(
    expect_equal(
      filter_proteins_by_proportion(input, min_prop = 0.75),
      output_75
    )
  )
  suppressMessages(
    expect_equal(
      filter_proteins_by_proportion(input, min_prop = 1),
      output_1
    )
  )
})
