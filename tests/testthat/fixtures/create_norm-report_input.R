...
# Data and annotations portions are simple,
# metadata is what matters here
test_metadata <- data.frame(sample_ID = paste0("sample", 1:52),
                            group = c(rep(c("A", "B", "C"), 17), "C"),
                            sex = c(rep(c("M", "F"), 25), "M", "M"),
                            treatment = c(rep("control", 26),
                                          rep("treatment", 26)))
rownames(test_metadata) <- test_metadata$sample_ID
set.seed(42)
test_data <- data.frame(a = as.integer(runif(n = 100, min = 10000, max = 20000)),
...
test_data_2 <- as.data.frame(rep(test_data, 9))[,1:52]
colnames(test_data_2) <- test_metadata$sample_ID
test_annotation <- data.frame(uniprot_id = paste0("protein", 1:100))
input <- DAList(data = test_data_2,
                annotation = test_annotation,
                metadata = test_metadata)
saveRDS(object = input, file = "tests/testthat/fixtures/norm_report_input_52samp.rds")
