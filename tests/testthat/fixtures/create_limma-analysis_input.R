...
saveRDS(object = input, file = "tests/testthat/fixtures/fit_limma_model_input.rds")


input_nonorm <- DAList(data = test_data,
                annotation = test_annotation,
                metadata = test_metadata) |>
  add_design(~ 0 + treatment)
saveRDS(object = input_nonorm, file = "tests/testthat/fixtures/fit_limma_model_input_nonorm.rds")


input_nodesign <- DAList(data = test_data,
                       annotation = test_annotation,
                       metadata = test_metadata)
saveRDS(object = input_nodesign, file = "tests/testthat/fixtures/fit_limma_model_input_nodesign.rds")
