...
functions, and hopefully does so in a way that makes the package easier
to use.
...
One thing to notes: this function just reads in the data and spits it
...
Specifying filtering is easier, and now works like core R functions like
`subset()` or `dplyr::filter()`: you just supply a logical expression
