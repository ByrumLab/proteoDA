...
After you add the statistical design, you fit the limma model with `fit_limma_model()` and get tables of differential expression results with `extract_DE_results()`. `fit_limma_model()` adds the `eBayes_fit` slot to the DIAlist, and `extract_DE_results()` adds the `results` slot as a list with one element for each statistical term/contrast. 
