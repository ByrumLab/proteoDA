...
![](images/Lou_proteiNorm_metrics.png){width=70%}
...
![](images/Lou_proteiNorm_MA.png){width=70%}
...
```{r example_volcano, eval=TRUE}
# Example: plot one contrast volcano in the vignette
example_contrast <- names(results$results)[1]
example_tab <- results$results[[example_contrast]]

library(ggplot2)
ggplot(example_tab, aes(x = logFC, y = -log10(P.Value))) +
  geom_point(alpha = 0.5) +
  geom_vline(xintercept = c(-logFC, logFC), linetype = "dashed") +
  geom_hline(yintercept = -log10(p.val), linetype = "dashed") +
  ggtitle(paste("Volcano plot:", example_contrast))
```

> This volcano plot shows the raw log2 fold-change vs. –log10 p-value for a single contrast.  
...
(You can leave this `eval=FALSE` initially; when debugging locally, turn it on.)
