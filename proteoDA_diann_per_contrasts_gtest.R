...
#results2 <- compute_movingSD_zscores(results)
#results2 <- compute_movingSD_zscores(results, binsize = 1000)

# This version gives names (compute_movingSD_zscores_v4.R)
names(results$tags$movingSDs$IDA_vs_DMSO)[1:5]
head(results$tags$movingSDs$IDA_vs_DMSO)[1:5]
head(results$tags$logFC_z_scores$IDA_vs_DMSO)[1:5]

# package version gives the values ran _v5.R
head(results$tags$movingSDs$IDA_vs_DMSO)[1:5]
...
statlist$IDA_vs_DMSO$logFC_z_scores["A0A024QZ89"]
results2$tags$logFC_z_scores$IDA_vs_DMSO["A0A024QZ89"]
