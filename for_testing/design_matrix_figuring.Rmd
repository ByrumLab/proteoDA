...
Above, the issue with this seemed to be in the calculation of the prior variance: they were slightly different across the different parameterizations. From doing some deep dives, here the issue seems to be with the unscaled standard deviations, and specifically for proteins that have some missing data. For some reason, we get slightly different unscaled SDs for those genes across our different parameterizations: genes with no missing data are the same.

Again, I'm not 100% sure how limma is dealing with this, would be good to dive in further and figure out what is "right". 


```{r, eval = F}
# Figuring out what is different. Seems to be the 
# Unscaled st deviation that is driving the differences:
waldo::compare(fit_noInt$eBayes_fit,
               efit)
# When we look at one of the rows where they are different:
waldo::compare(fit_noInt$eBayes_fit$stdev.unscaled,
               efit$stdev.unscaled)
# E.g., row 1987
rbind(fit_noInt$eBayes_fit$stdev.unscaled[1987,],
      efit$stdev.unscaled[1987,])
# We see differences in the SD for any comparison involving groups 3d and 5d
# When we look at the data for that row:
fit_noInt$data[1987,]

# Samples from 3d and 5d have missing data.
# Results from non-control comparisons that don't involve groups 3d or 5d are the same:
# basically the same: the main stats are all the same, though the adjusted Ps are a little
# different (since they depend on all the other p values)
all.equal(results_intercept$stats_by_contrast$NRBC_7d_vs_NRBC_1d[1987,],
          results_noInt$stats_by_contrast$NRBC_7d_vs_NRBC_1d[1987,])

# So, it just seems to be an issue around missing data. 
```
...
* The order of specifying model terms matters in the no-intercept model. 

* For models with only one factor, the choice of model parameterization seems to matter only in cases where there is missing data: scaled standard deviations are calculated slightly differently. 
